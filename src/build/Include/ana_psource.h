...
!
!! svn $Id: ana_psource.h 2328 2014-01-23 20:16:18Z arango $
!!======================================================================
...
!          SOURCES(ng)%Jsrc(Nsrc(ng))=120   !ckh: for high-resolution
        END IF
...
!        DO is=1,(Nsrc(ng)-1)/2               ! North end
        DO is=1,Nsrc(ng)-1         ! Only one wall of rivers; ckharris 3/14
          i=SOURCES(ng)%Isrc(is)
          j=SOURCES(ng)%Jsrc(is)
          IF (((IstrT.le.i).and.(i.le.IendT)).and.                      &
     &        ((JstrT.le.j).and.(j.le.JendT))) THEN
!this hack made no difference ... (jmm)
!            SOURCES(ng)%Qbar(is)=-0.05_r8*om_v(i,j)*                    &
!     &                           (0.5_r8*(h(i,j-1)+h(i  ,j)))
            SOURCES(ng)%Qbar(is)=-0.05_r8*om_v(i,j)*                    &
...
# elif defined RIVERPLUME2
        IF (DOMAIN(ng)%NorthEast_Test(tile)) THEN
          DO k=1,N(ng)
            ...
            SOURCES(ng)%Tsrc(Nsrc(ng),k,isalt+1)=1.0_r8
            SOURCES(ng)%Tsrc(Nsrc(ng),k,isalt+2)=1.0_r8
            SOURCES(ng)%Tsrc(Nsrc(ng),k,isalt+3)=0.0_r8
          END DO
        END IF
# else
